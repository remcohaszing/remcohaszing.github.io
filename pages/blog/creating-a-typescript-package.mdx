...
package can’t be consumed by TypeScript users who want to use this package. `declarationMap` emits
a declaration map for each declaration file. If this is included, _Go to definition_ from within an
...
Note that in our TextDecoderStream, we import `greet` from `my-package`. We could have imported
`greet` from `./my-package.js`. However, by importing from it `my-package` we do not only test the
`greet` function, we also make sure that it is properly exported in the public interface.
